...
def segments_openai_advanced_tts(filtered_openai_tts_segments, TRANSLATE_AUDIO_TO):
    from openai import OpenAI
    
    # Initialize client
    client = OpenAI()
    
    # Instructions provided in your request (You can also make this dynamic later if needed)
    instructions = """Accent/Affect: Warm, refined, and gently instructive, reminiscent of a friendly art instructor.\n\nTone: Calm, encouraging, and articulate, clearly describing each step with patience.\n\nPacing: Slow and deliberate, pausing often to allow the listener to follow instructions comfortably.\n\nEmotion: Cheerful, supportive, and pleasantly enthusiastic; convey genuine enjoyment and appreciation of art.\n\nPronunciation: Clearly articulate artistic terminology (e.g., \"brushstrokes,\" \"landscape,\" \"palette\") with gentle emphasis.\n\nPersonality Affect: Friendly and approachable with a hint of sophistication; speak confidently and reassuringly, guiding users through each painting step patiently and warmly.\n\nsubject related to gams"""

    # Iterate through every sentence/segment
    for segment in tqdm(filtered_openai_tts_segments["segments"]):
        speaker = segment["speaker"]
        text = segment["text"].strip()
        start = segment["start"]
        tts_name = segment["tts_name"] # Contains the voice selection
        
        # Parse the voice from the UI selection string (e.g., ">alloy ...")
        # If your UI string is ">gpt-4o-mini-tts OpenAI-Advanced", we might default to "echo" 
        # or parse a specific voice if you allow voice selection for this model.
        # For this example, we use "echo" as requested, or map speakers to specific voices.
        voice_id = "echo" 

        # dubbing expects files to be named by their start time
        filename = f"audio/{start}.mp3" 
        
        logger.info(f"{text} >> {filename}")

        try:
            # YOUR NEW API CODE IMPLEMENTATION
            with client.audio.speech.with_streaming_response.create(
                model="gpt-4o-mini-tts", # The specific model you requested
                voice=voice_id,
                input=text, # The text from the current segment
                instructions=instructions,
            ) as response:
                response.stream_to_file(filename)
            
            # dubbing Post-processing:
            # We need to verify the file exists and is valid
            verify_saved_file_and_size(filename)
            
            # Optional: Convert to OGG/Vorbis if the rest of the pipeline strictly requires .ogg
            # Ideally, dubbing handles mp3, but to be safe consistent with other functions:
            data, sample_rate = sf.read(filename)
            # Apply padding if necessary (helper function in existing code)
            data = pad_array(data, sample_rate) 
            
            # Overwrite as OGG for internal consistency (optional but recommended)
            final_filename = f"audio/{start}.ogg"
            write_chunked(
                file=final_filename,
                samplerate=sample_rate,
                data=data,
            ...
            # Remove the intermediate mp3
            if os.path.exists(filename):
                os.remove(filename)

        except Exception as error:
            # Fallback logic defined in the existing code
            error_handling_in_tts(error, segment, TRANSLATE_AUDIO_TO, filename)
...
def segments_openai_tts(
    ...
    # Your provided instructions string
    instructions = """Accent/Affect: Warm, refined, and gently instructive, reminiscent of a friendly art instructor.\n\nTone: Calm, encouraging, and articulate, clearly describing each step with patience.\n\nPacing: Slow and deliberate, pausing often to allow the listener to follow instructions comfortably.\n\nEmotion: Cheerful, supportive, and pleasantly enthusiastic; convey genuine enjoyment and appreciation of art.\n\nPronunciation: Clearly articulate artistic terminology (e.g., \"brushstrokes,\" \"landscape,\" \"palette\") with gentle emphasis.\n\nPersonality Affect: Friendly and approachable with a hint of sophistication; speak confidently and reassuringly, guiding users through each painting step patiently and warmly.\n\nsubject related to gams"""
    ...
    for segment in tqdm(filtered_openai_tts_segments["segments"]):
        ...
        # dubbing expects the filename to be the start time in the audio/ folder
        # We generally use .ogg or .mp3. Your snippet produces mp3 stream.
        filename = f"audio/{start}.mp3" 
        logger.info(f"{text} >> {filename}")

        # Determine voice from selection (e.g., ">echo OpenAI-TTS" becomes "echo")
        # If tts_name is ">gpt-4o-mini-tts OpenAI-TTS", we default to "echo" or parse logic if you add voice selection later.
        voice_selection = "echo" # Default based on your snippet
        
        # If the user selected a specific voice in the UI (like >alloy), extract it
        if "OpenAI-TTS" in tts_name and not "gpt-4o-mini-tts" in tts_name:
             voice_selection = tts_name.split()[0][1:]

        try:
            # YOUR NEW API IMPLEMENTATION
            with client.audio.speech.with_streaming_response.create(
                model="gpt-4o-mini-tts", # Using the model you specified
                voice=voice_selection, 
                input=text, # The segment text
                instructions=instructions, # Your custom instructions
            ) as response:
                response.stream_to_file(filename)
            ...
            # Fallback logic provided by dubbing
            error_handling_in_tts(error, segment, TRANSLATE_AUDIO_TO, filename)
...
def audio_segmentation_to_voice(
    ...
    pattern_openai_advanced = re.compile(r".* OpenAI-Advanced$") # <--- ADD THIS
    ...
    speakers_openai_advanced = find_spkr(pattern_openai_advanced, speaker_to_voice, all_segments)
    ...
    filtered_openai_advanced = filter_by_speaker(speakers_openai_advanced, all_segments)
    ...
    if filtered_openai_advanced["segments"]:
        logger.info(f"OpenAI Advanced TTS: {speakers_openai_advanced}")
        segments_openai_advanced_tts(filtered_openai_advanced, TRANSLATE_AUDIO_TO)
